...
function range_select() {
    $.ajax({
        type: "post",
        url: "../usdt/SysRiseFall/SysRiseFallSelect",
        async: true,
        dataType: "json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', token);
        ...
        success: function (info) {
            ...
            if (info.code == 100) {
                ...
                for (var i in info.range) {
                    ...
                    var system_range_name = "";
                    for (var j in system_range) {
                        var s_range = system_range[j];
                        if (s_range.pid == range.sys_range_id) {
                            var name;
                            if (s_range.style == 0) {
                                name = "涨";
                            } else {
                            ...
                            system_range_name = name + s_range.range + "%";
                        }
                    }
                    str += "<td>" + system_range_name + "</td>";
                    if (range.state == 0) {
...
function add() {
    ...
    for (var i in system_minute) {
        ...
        if (i == 0) {
            sys_minute_id = sys_minute.pid;
        }
        str += "<option value=" + sys_minute.pid + ">" + style_name + ":" + sys_minute.name + "</option>";
    }
    $("#minute").html(str);
    var str1;
    for (var i in system_range) {
        var range = system_range[i];
        if (sys_minute_id == range.sys_minute_id) {
            var name;
            if (range.style == 0) {
                name = "涨";
            } else {
            ...
            var system_range_name = name + range.range + "%";
            str1 += "<option value=" + range.pid + ">" + system_range_name + "</option>";
...
function qd_add() {
    ...
    var sys_range_id = $("#sys_range").val();
    var range = $("#range").val();
    $.ajax({
        type: "post",
        url: "../usdt/SysRiseFall/SysRiseFallInsert",
        async: true,
        dataType: "json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', token);
        },
        data: {sys_minute_id: minute, range: range, sys_range_id: sys_range_id},
        success: function (info) {
            if (info.code == 100) {
                range_select();
                $("#addFile").modal("hide");
...
function update() {
    ...
    if (t != false) {
        ...
        for (var j in system_rise_fall) {
            var range = system_rise_fall[j];
            if (range.pid == t[0]) {
                var str;
                for (var i in system_minute) {
                    var sys_minute = system_minute[i];
                ...
                var str1 = "";
                for (var i in system_range) {
                    var s_range = system_range[i];
                    if (s_range.sys_minute_id == range.sys_minute_id) {
                        var name;
                        if (s_range.style == 0) {
                            name = "涨";
                        } else {
                        ...
                        var system_range_name = name + s_range.range + "%";
                        if (s_range.sys_range_id == s_range.pid) {
                            str1 += "<option value=" + s_range.pid + " selected>" + system_range_name + "</option>";
                        } else {
                            str1 += "<option value=" + s_range.pid + ">" + system_range_name + "</option>";
                        }
                    }
                }
                $("#sys_range").html(str1);
...
function qd_update() {
    ...
    $.ajax({
        type: "post",
        url: "../usdt/SysRiseFall/SysRiseFallUpdate",
        async: true,
        dataType: "json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', token);
        },
        data: {sys_minute_id: minute, range: range, pid: t[0], sys_range_id: sys_range_id},
        success: function (info) {
            if (info.code == 100) {
                range_select();
                $("#addFile").modal("hide");
...
function qd_del(state) {
    ...
    $.ajax({
        type: "post",
        url: "../usdt/SysRiseFall/SysRiseFallDel",
        async: true,
        dataType: "json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', token);
...
function range_update(pid) {
    for (var i in system_range) {
        ...
        var system_range_name = name + range.range + "%";
        str += "<option value=" + range.pid + ">" + system_range_name + "</option>";
